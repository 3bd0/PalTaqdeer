...
@app.route('/')
def index():
    query = "select distinct * from student_forecast"
    cur.execute(query)
    data = list(cur)
...
def result():
    ...
    if final_grade_rounded > 20:
        final_grade_rounded = 20

    if final_grade_rounded < 0:
        final_grade_rounded = 0

    query = f""" INSERT INTO student_forecast (age, medu, fedu, failures, higher, romantic, g1, g2, gouout, g3)
            VALUES ('{age}','{medu}','{fedu}','{failures}','{higher}','{romantic}','{g1}','{g2}','{gouout}', '{final_grade_rounded}'); """
